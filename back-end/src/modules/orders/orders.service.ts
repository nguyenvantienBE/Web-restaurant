...
export class OrdersService {
  ...
  async updateItemStatus(itemId: string, status: ItemStatus) {
    ...
    const validTransitions: Record<ItemStatus, ItemStatus[]> = {
      ...
      SERVED: [], // Cannot transition out of served
      CANCELLED: [], // Cannot transition out of cancelled
    };

    if (!validTransitions[item.status].includes(status)) {
      throw new BadRequestException(`Cannot change item status from ${item.status} to ${status}`);
    }
    ...
    const hasCooking = allItems.some((i) => i.status === 'COOKING');
    ...
    } else if (hasReady && !hasCooking && !hasNew) {
      newOrderStatus = 'READY';
    } else if (hasCooking || hasReady) {
    ...
    if (newOrderStatus !== item.order.status) {
      ...
      item.order.status = newOrderStatus;
    }
